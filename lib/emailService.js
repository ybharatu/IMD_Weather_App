...
function isConfigured() {
    return Boolean(SMTP_USER && SMTP_PASS) || gmailApiConfigured();
}
...
function describeTransport() {
    ...
    if (Boolean(SMTP_USER && SMTP_PASS)) {
        return `SMTP via ${SMTP_HOST}:${SMTP_PORT}`;
    }
    return 'NONE (set SMTP_* or GMAIL_* vars)';
}
...
async function getGmailAccessToken() {
    ...
    if (!data.access_token) {
        throw new Error('No access_token in Gmail token response');
    }
    return data.access_token;
}

function buildRawMessage({ to, subject, html, text }) {
    const boundary = `IMD_WX_${Date.now().toString(36)}${Math.random().toString(36).slice(2)}`;
...
async function sendEmail({ to, subject, html, text }) {
    ...
    if (gmailApiConfigured()) {
        try {
            ...
            console.error(`[email] Gmail API failed, falling back to SMTP: ${err.message}`);
        }
    }
